...
def _alias_in_canonical_context(
    ...
    for cm in re.finditer(rf"\b{re.escape(canonical_term)}\b", window, re.IGNORECASE):
            ...
            for g in glossary:
                for alias in g.aliases_rejected:
                    for m in re.finditer(
                        rf"\b{re.escape(alias)}\b", content, re.IGNORECASE
                    ):
                        if _alias_in_canonical_context(
                            content, m.start(), m.end(), g.term
                        ):
                            continue
