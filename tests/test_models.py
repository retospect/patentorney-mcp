...
class TestClaim:
    ...
    def test_dependent_claim(self):
        c = Claim(
            id="method-1a", type="dependent", category="method", depends_on="method-1"
        )
        assert c.depends_on == "method-1"

    def test_independent_with_depends_on_rejected(self):
        with pytest.raises(
            ValidationError, match="Independent claims must have depends_on=null"
        ):
            Claim(
                id="bad", type="independent", category="method", depends_on="something"
            )

    def test_dependent_without_depends_on_rejected(self):
        with pytest.raises(
            ValidationError, match="Dependent claims must specify depends_on"
        ):
